...
/// A very mild convenience reader around Mach-O files, handling
/// endianness and buffering and collecting a set of modifications.
/// Actual Mach-O parsing beyond the magic is left to the user.
///
/// Note that Mach-O files cannot meaningfully exceed 4 GB because all
/// internal fields are u32, even on 64-bit platforms.
#[derive(Debug)]
...
impl MachOManipulator {
    ...
    /// Read several u32s in the current endianness, advancing the
    /// cursor.
    fn read_u32s<const N: usize>(&mut self) -> Result<[u32; N]> {
        self.offset += 4 * N;
        let mut result = [0; N];
        match self.endian {
            Endian::Little => reader.read_u32_into::<byteorder::LittleEndian>(&mut result)?,
            Endian::Big => reader.read_u32_into::<byteorder::Bigndian>(&mut result)?,
        };
        result
        self.endian.read_u32s(self.reader)
    }
...
impl Pages {
    fn new(file: &mut MachOManipulator, page_shift: u32) -> Result<Self> {
        if page_shift == 0 {
            // This means the page size is "infinite" which I think is
            // one big page? The kernel doesn't allow this anyway. We
            // could implement this if needed, check other tools'
            // behavior.
    ...
    fn page_at(&mut self, offset: usize) -> Result<&mut Vec<u8>> {
        ...
        // read_exact_at is documented as leaving the _entire_ contents
        // of the buffer unspecified if it hits EOF before filling. We
        // specifically are okay with an incomplete last page though.
        let mut n = 0;
    ...
    fn modify(&mut self, modification: (usize, Vec<u8>)) -> Result<()> {
        ...
        let (this_modification, next_modification) = if relative_end >
        self.page_length


        let contents = if relative_end > self.page_length() {
            let (ours, excess) = contents.split_at(relative_end - self.page_length());
    ...
    pub fn get_modified_pages(self) -> Vec<Page> {
        for modification in self.file.modifications {
...
fn patch_dylib_install_name_ourselves(dylib: PathBuf) -> Result<(), Error> {
    // In theory, this is straightforward: find the LC_ID_DYLIB load
    // command, make sure there's enough room for the updated string,
    // and overwrite the string.
    //
    // However, on recent versions of macOS on Apple silicon (at least),
    // there's a requirement for every binary to have a valid
    // LC_CODE_SIGNATURE. This doesn't need to be an actual _signature_;
    // it can be an "ad-hoc signature" consisting of just hashes of the
    // file. The Apple-provided linker, as well as other tools that
    // output binaries like install_name_tool and strip, generate an
    // ad-hoc signature and set the "linker-signed" flag, indicating
    // there is nothing interesting about the signature (like
    // entitlements) you should worry about losing if the signature is
    // regenerated. We need to do the same thing.
    //
    // While there is a platform library (libcodedirectory.dylib) to
    // generate the signature, and the source is public (it's vendored
    // into ld64 on Apple's OSS drops), it's geared towards generating a
    // signature from scratch when you understand the full binary; it
    // doesn't have a method to load and modify an existing signature,
    // keeping all the other settings the same.  Fortunately, the parts
    // of the signature we need to understand aren't hard:
    // - The signature is located at the end of the file, pointed to by
    //   an LC_CODE_SIGNATURE linker command.
    // - All the pages in the file up to that signature are hashed, one
    //   page at a time.
    // - These hashes are stored, along with other info, in a "code
    //   directory" data structure.
    // - The code directory is one of several possible "blobs" in the
    //   signature. There may be multiple if you want to support
    //   multiple hash types; there may be other data like entitlements
    //   or an actual signature.
    // - These blobs are themselves stored in a "superblob" data
    //   structure, which is what is placed at the end of the file.
    // So, once we know what page(s) we modified while editing
    // LC_ID_DYLIB, we rehash those pages, find where they're located in
    // the code directory, and overwrite the hashes. Because we're
    // requiring that the file is linker-signed, nothing attests to the
    // hashes and we don't need to make further changes.
    //
    // TODO(geofft): Support universal Mach-O files, there are some good
    ...
    // Just to check that we didn't misparse anything, we will check
    // after reading the load commands that we read sizecmds bytes.
    ...
    for i in 0..ncmds {
        ...
        if cmd == LC_ID_DYLIB {
            let [name_offset, _timestamp, _current_version, _compatibility_version] = f.read_u32s()?;
    ...
    let Some(offset, bufsize) = found_id_dylib else {
        // TODO(geofft): We could add one between the last load command
        // and the start of the first segment? Is that hard?
    ...
    if let Some(dataoff, datasize) = found_code_signature {
        ...
        for (typ, offset) in blobs {
            ...
            // Figure out which pages we need to rehash
            ...
            let page_length = 1 << pageSize;
            let page_mask = ~(page_length - 1);
            for modification in in &f.modifications {
                let mut remaining = Some(modification);
                while let Some((offset, contents)) = remaining {
                    let index = offset >> pageSize;
                    let mut page = if let Some(page) = pages.iter_mut().find(|&x| x.index == index) {
                        page.contents
                    } else {
                        let mut contents = vec![0u8; page_length];
                        // read_exact_at is documented as leaving the
                        // _entire_ contents of the buffer unspecified
                        // if it hits EOF before filling. We are okay
                        // with an incomplete last page though.
                        let mut n = 0;
                        while n < contents.len() {
                            match f.read_at(&mut buf[n..], offset + n) {
                                Ok(0) => break,
                                Ok(m) => {n += m;}
                                Err(ref e) if e.is_interrupted() => {},
                                Err(e) => return Err(e),
                        ...
                        let page = Page { index, contents };
                        pages.push(page);
                        pages.last_mut().contents
                    }
                    let relative_start = offset & page_mask;
                    let relative_end = relative_start + contents.len();
                    let contents = if relative_end > page_length {
                        let (ours, excess) = contents.split_at(relative_end - page_length);
                        remaining = Some(offset.next_multiple_of(page_length), excess);
                        ours
                    } else {
                        remaining = None;
                        contents
                ...
                let page_index = offset >> pageSize;
                let mut contents = if let Some(page) = self.pages.iter_mut().find(|&x| x.index == index) {
                    page.contents
                } else {
                    let mut contents = vec![0u8; 1 << self.page_shift];
                    
                    let contents = f.read_at(offset & page_mask
                    todo!() as Vec<u8>;
                    let page = Page { index, contents };
                    self.pages.push(page);
                    self.pages.last_mut().contents
                    }
                pages.modify(modification);
            }

            // Rehash the pages
            for Page { index, contents } in pages.pages {
                if index >= nCodeSlots {
